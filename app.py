...
# SQLite 데이터베이스 설정
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///game_history.db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False # 객체 변경 추적 기능 비활성화
...
class GameHistory(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    player_choice = db.Column(db.String(80), nullable=False)
    computer_choice = db.Column(db.String(80), nullable=False)
...
@app.route('/')
def index():
    # 세션에 'win', 'lose', 'draw' 키가 없는 경우 초기화
    if 'win' not in session:
        session['win'] = 0
    if 'lose' not in session:
        session['lose'] = 0
